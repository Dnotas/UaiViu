...
export const createPublicOrder = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (!req.body.customerName?.trim()) throw new AppError("Nome é obrigatório", 400);
  if (orderType === "delivery" && !req.body.cep?.trim()) throw new AppError("CEP é obrigatório", 400);
  if (orderType === "delivery" && !customerAddress?.trim()) throw new AppError("Endereço é obrigatório", 400);
