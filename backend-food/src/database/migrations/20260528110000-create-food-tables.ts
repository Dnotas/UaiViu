...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    ...
    await queryInterface.createTable("food_whatsapps", {
      ...
      status: { type: DataTypes.ENUM("DISCONNECTED", "OPENING", "QRCODE", "TIMEOUT", "CONNECTED"), defaultValue: "DISCONNECTED" },
    ...
    await queryInterface.createTable("food_orders", {
      ...
      status: { type: DataTypes.ENUM("pending", "confirmed", "preparing", "on_the_way", "delivered", "cancelled"), defaultValue: "pending" },
      paymentMethod: { type: DataTypes.ENUM("credit", "debit", "pix", "cash") },
      paymentStatus: { type: DataTypes.ENUM("pending", "paid", "failed"), defaultValue: "pending" },
      deliveryToken: { type: DataTypes.UUID },
      orderType: { type: DataTypes.ENUM("delivery", "pickup"), defaultValue: "delivery" },
    ...
    await queryInterface.createTable("food_payment_configs", {
      ...
      pixKeyType: { type: DataTypes.ENUM("cpf", "cnpj", "email", "phone", "random"), defaultValue: "random" },
